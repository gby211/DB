...
from PyQt5.QtWidgets import QTableWidgetItem, QAbstractScrollArea, QMessageBox
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.comboBox.addItem("log_table")
        ...
        self.pushButtonPrice = QtWidgets.QPushButton(self.centralwidget)
        self.pushButtonPrice.setMaximumSize(QtCore.QSize(180, 16777215))
        self.pushButtonPrice.setObjectName("pushButtonPrice")
        self.formLayout.setWidget(9, QtWidgets.QFormLayout.LabelRole, self.pushButtonPrice)

        self.pushButtonSledDostavka = QtWidgets.QPushButton(self.centralwidget)
        self.pushButtonSledDostavka.setMaximumSize(QtCore.QSize(180, 16777215))
        self.pushButtonSledDostavka.setObjectName("pushButtonSledDostavka")
        self.formLayout.setWidget(10, QtWidgets.QFormLayout.LabelRole, self.pushButtonSledDostavka)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButtonPrice.setText(_translate("MainWindow", "Посмотреть цену\nвсех заказов"))
        self.pushButtonPrice.clicked.connect(self.on_click_pushButtonPrice)

        self.pushButtonSledDostavka.setText(_translate("MainWindow", "Посмотреть\nследующий заказ"))
        self.pushButtonSledDostavka.clicked.connect(self.on_click_pushButtonSledDostavka)
    ...
    def on_click_pushButtonNew(self):
        ...
        self.cursor.execute(show_db_query)
        # self.tableView.setRowCount(len(cursor.column_names))
        # self.tableView.setColumnCount(len(self.cursor.description))
        i = 0
        # self.tableView.setHorizontalHeaderLabels(self.cursor.column_names)
        print(self.cursor.column_names)
        print(type(self.cursor.column_names))

        self.fields = []
        self.name_col = self.cursor.column_names
        i = 1
        for n in self.cursor.column_names:
            lineEdit12 = QtWidgets.QLineEdit(self.centralwidget)
            lineEdit12.setText("")

            lineEdit12.setObjectName("lineEdit"+str(i))
            lineEdit12.setPlaceholderText(n)
            lineEdit12.setMinimumSize(QtCore.QSize(0, 20))
    ...
    def on_click_pushButtonNewNew(self):
        ...
        try:
            ...
            if len(fields1) == len(self.name_col):
                show_db_query = "INSERT INTO " + "vegetable_production_db." + str(self.curentTable) + f" ({', '.join(self.name_col)})" + f" VALUES ({', '.join(fields1)});"
            else:
                list1 = list(self.name_col)
                list1.pop(0)
                show_db_query = "INSERT INTO " + "vegetable_production_db." + str(self.curentTable) + f" ({', '.join(list1)})" + f" VALUES ({', '.join(fields1)});"
            print(show_db_query)
    ...
    def on_click_pushButtonSledDostavka(self):

        self.tableView.clear()
        self.tableView.setRowCount(0)
        self.tableView.setColumnCount(0)
        show_db_query = "select * from vegetable_production_db.delivery where delivery.date > current_date() order by date limit 1;"

        self.cursor.execute(show_db_query)
        # self.tableView.setRowCount(len(cursor.column_names))
        self.tableView.setColumnCount(len(self.cursor.description))
        i = 0
        self.tableView.setHorizontalHeaderLabels(self.cursor.column_names)#.setStretchLastSection(True)
        self.tableView.verticalHeader().setVisible(False)

        for r,db in enumerate(self.cursor):
            print(db)
            print(type(db))
            self.tableView.insertRow(r)
            for c,data in enumerate(db):
    ...
    def on_click_pushButtonPrice(self):

        self.tableView.clear()
        self.tableView.setRowCount(0)
        self.tableView.setColumnCount(0)
        show_db_query = "select sum(price) 'Sum', max(price) 'max price', min(price) 'min price', avg(price) 'avg price' from vegetable_production_db.order;"
